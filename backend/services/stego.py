...
from stego_core.preprocessing import encode_img_to_bytes, load_img_from_bytes
from stego_core.pseudorandom import generate_pixel_coordinates, password_to_seed
...
def run_embed(
    ...
    pixel_coords: list[tuple[int, int]] = generate_pixel_coordinates(
        height, width, seed
    )

    stego_img: np.ndarray = embed_message(
        img, message, classification_map, pixel_coords
    )
